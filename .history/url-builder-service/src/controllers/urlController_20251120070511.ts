...
exports.buildUrl = async (req : Request, res : Response) => {
    try {
        ...
        let userEmail = "";
        try {
            // We use axios.patch<ResponseData> to define what we expect back
            const authResponse = await axios.patch(`${AUTH_MS_URL}/internal/update-url`, {
                name: name,
                url: generatedUrl
            });
            userEmail = authResponse.data.email;
        } catch (err: any) { // 5. Typed Catch Block
            // 'err' is unknown type in TS, we cast to 'any' to access .message
            console.error("Auth MS Error:", err.message);
