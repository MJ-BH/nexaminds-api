...
if (validTimestamps.length === 0) {
    // Only fail if EVERYTHING was invalid
    return res.status(400).json({ error: 'No valid timestamps provided (must be positive integers).' });
        ...
        const generatedUrl  = `https://server/record?name=${name}&duration=${formatDuration(durationVal)}&median=${formatDuration(medianVal)}`;
